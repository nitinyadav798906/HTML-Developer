...
<head>
    ...
    <style>
        ...
        .lamp-container {{ cursor: pointer; position: relative; scale: 0.9; }}
        ...
        .login-box {{ background: rgba(255,255,255,0.05); padding: 30px; border-radius: 20px; backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.1); text-align: center; width: 90%; max-width: 320px; opacity: 0.2; transition: 0.5s; color: white; }}
        .login-box.active {{ opacity: 1; }}
        .login-box input {{ width: 100%; padding: 12px; margin: 15px 0; border: 1px solid #444; border-radius: 8px; background: #222; color: #fff; outline: none; }}
        ...
        * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
        ...
        .player-header {{ display: flex; justify-content: space-between; padding: 12px 15px; color: white; background: linear-gradient(to bottom, rgba(0,0,0,0.9), transparent); position: absolute; top: 0; width: 100%; z-index: 20; cursor: move; }}
        .overlay-seek {{ position: absolute; top: 50%; transform: translateY(-50%); background: rgba(0,0,0,0.5); color: white; padding: 10px 15px; border-radius: 30px; font-size: 14px; cursor: pointer; z-index: 15; }}
        .seek-left {{ left: 20px; }} .seek-right {{ right: 20px; }}
        ...
        #watermark {{ position: absolute; top: 10%; right: 5%; color: rgba(255,255,255,0.2); font-weight: 900; pointer-events: none; z-index: 20; font-size: 18px; }}
...
<body>
    ...
    <div id="app-wrapper">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-box" onclick="filterList('FAV')"><span class="stat-num" id="favCount">0</span>Fav ❤️</div>
            <div class="stat-box" onclick="filterList('VIDEO')"><span class="stat-num">{v_c}</span>Video</div>
            <div class="stat-box" onclick="filterList('AUDIO')"><span class="stat-num">{a_c}</span>Audio</div>
            <div class="stat-box" onclick="filterList('PDF')"><span class="stat-num">{p_c}</span>PDF</div>
            <div class="stat-box" onclick="filterList('IMAGE')"><span class="stat-num">{i_c}</span>Img</div>
        </div>
        ...
        <div id="vModal" class="modal">
            <div class="modal-content" id="modalContent">
                ...
                <div class="media-box" id="mediaBox" style="min-height:250px; background:#000; position:relative; display:flex; align-items:center; justify-content:center;">
                    ...
                    <div id="vidContainer" style="width:100%; display:none;">
                        <div class="overlay-seek seek-left" onclick="player.currentTime -= 10">⏪ 10s</div>
                        <div class="overlay-seek seek-right" onclick="player.currentTime += 10">10s ⏩</div>
                ...
                <div class="controls-row" id="vidControls" style="display:none;">
                    ...
                    <button class="c-btn" onclick="toggleFav()">❤️ Fav</button>
                </div>
            </div>
        </div>
        <div id="toast"></div>
    ...
    <script>
        ...
        let currentIndex = -1; let isMinimized = false;
        ...
        document.getElementById('mediaBox').addEventListener('touchmove', e => {{
            if(e.touches[0].clientX > window.innerWidth / 2) {{
                ...
                const vInd = document.getElementById('volIndicator'); vInd.style.display = 'block';
                vInd.innerText = 'Vol: ' + Math.round(player.volume * 100) + '%';
...
async def process_file(c, m):
    ...
    await m.reply_document(out_path, caption="🔥 Done!")
