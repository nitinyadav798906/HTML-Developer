...
def generate_html(file_name, content, is_protected=False):
    ...
    for idx, (name, url) in enumerate(raw_lines):
        ...
        # FIXED: Double curly brackets for HTML template
        items_html += f'''
...
<head>
    ...
    <style>
        ...
        .lamp-stick {{ width: 6px; height: 100px; background: #fff; margin: 0 auto; }}
        .lamp-base {{ width: 70px; height: 12px; background: #fff; border-radius: 10px 10px 0 0; margin: 0 auto; }}
        .light-glow {{ position: absolute; top: 20px; left: 50%; transform: translateX(-50%); width: 250px; height: 250px; background: radial-gradient(circle, rgba(255,255,200,0.3) 0%, transparent 70%); opacity: 0; transition: 0.5s; pointer-events: none; }}
        ...
        .login-box input {{ width: 100%; padding: 12px; margin: 15px 0; border: 1px solid #444; border-radius: 8px; background: #222; color: #fff; }}
        ...
        * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
        ...
        .player-header {{ display: flex; justify-content: space-between; padding: 12px 15px; color: white; background: rgba(0,0,0,0.9); position: absolute; top: 0; width: 100%; z-index: 20; cursor: move; }}
        ...
        #watermark {{ position: absolute; top: 10%; right: 5%; color: rgba(255,255,255,0.3); font-weight: 900; pointer-events: none; z-index: 20; font-size: 18px; }}
        ...
        body.minimized .modal {{ background: transparent !important; pointer-events: none; }}
        body.minimized .modal-content {{ pointer-events: auto; width: 300px !important; position: fixed; bottom: 80px; right: 20px; border: 1px solid #444; border-radius: 12px; overflow: hidden; }}
    </style>
</head>
<body>
    {login_html}
    <div id="app-wrapper">
        <div class="header">
            <div class="top-row">
                ...
                <div onclick="if(confirm('Clear Data?')) {{ localStorage.clear(); location.reload(); }}" style="cursor:pointer;">🗑️</div>
            </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-box" onclick="filterList('FAV')" style="color:#ef4444;"><span class="stat-num" id="favCount">0</span>Fav ❤️</div>
        ...
        <div id="vModal" class="modal">
            <div class="modal-content" id="modalContent">
                <div class="player-header" id="dragHandle">
                    ...
                    <div><span onclick="toggleMinimize()" style="cursor:pointer; margin-right:15px;">📉</span><span onclick="closeModal()" style="cursor:pointer; color:#ff4d4d; font-weight:bold;">✕</span></div>
                </div>
                <div class="media-box" id="mediaBox" style="min-height:250px; background:#000; position:relative; display:flex; align-items:center; justify-content:center;">
                    <div id="watermark">{BOT_OWNER_NAME}</div>
                    <div id="volIndicator">Vol: 100%</div>
                    <div id="vidContainer" style="width:100%; display:none;"><video id="player" playsinline controls style="width:100%;"></video></div>
                    <iframe id="pdfFrame" class="pdf-frame" style="display:none; width:100%; height:80vh; border:none;"></iframe>
                    <img id="imgView" style="display:none; max-width:100%; max-height:80vh;">
                </div>
    ...
    <script>
        let isLampOn = false; const playlist = {js_playlist}; let currentIndex = -1; let isMinimized = false;
        const player = new Plyr('#player', {{ quality: {{ default: 1080, options: [4320, 2160, 1440, 1080, 720, 480] }} }});
        ...
        function toggleLampUI() {{
            ...
            if(isLampOn) {{ gsap.to(screen, {{ backgroundColor: "#1c1f24", duration: 0.6 }}); box.classList.add('active'); glow.style.opacity = "1"; }}
            else {{ gsap.to(screen, {{ backgroundColor: "#121417", duration: 0.6 }}); box.classList.remove('active'); glow.style.opacity = "0"; }}
        }}
        function checkPass() {{
            if(document.getElementById('passInput').value === "{SKY_PASSWORD}") {{ document.getElementById('login-screen').style.display = 'none'; document.getElementById('app-wrapper').style.display = 'block'; }}
        ...
        function openContent(idx) {{
            ...
            document.getElementById('vidContainer').style.display = document.getElementById('pdfFrame').style.display = document.getElementById('imgView').style.display = 'none';
            if(item.type === 'VIDEO' || item.type === 'AUDIO') {{
                document.getElementById('vidContainer').style.display = 'block'; document.getElementById('vidControls').style.display = 'flex';
                if(item.url.includes('.m3u8')) {{ const hls = new Hls(); hls.loadSource(item.url); hls.attachMedia(document.querySelector('#player')); }}
                else document.querySelector('#player').src = item.url;
                player.play();
            }} else if(item.type === 'PDF') {{ document.getElementById('pdfFrame').style.display = 'block'; document.getElementById('pdfFrame').src = "https://docs.google.com/gview?embedded=true&url=" + encodeURIComponent(item.url); }}
            else if(item.type === 'IMAGE') {{ document.getElementById('imgView').style.display = 'block'; document.getElementById('imgView').src = item.url; }}
        }}
        ...
        function filterList(t) {{ document.querySelectorAll('.item-card').forEach(el => {{ el.style.display = (t === 'all' || el.getAttribute('data-type') === t) ? 'flex' : 'none'; }}); }}
        function searchList() {{ const v = document.getElementById('searchInput').value.toLowerCase(); document.querySelectorAll('.item-card').forEach(el => {{ el.style.display = el.innerText.toLowerCase().includes(v) ? 'flex' : 'none'; }}); }}
    </script>
</body>
</html>
"""

# ================= TELEGRAM HANDLER (UNTOUCHED) =================
@app.on_message(filters.command(["start", "stop", "html", "sky", "txt"]))
async def handle_cmds(c, m):
    cmd = m.command[0]
    if cmd == "start": return await m.reply_text(f"🔥 Pro Player Bot\n\n/html - Normal\n/sky - Secured\n/txt - Links")
...
async def process_file(c, m):
    ...
    if mode in ["html", "sky"]:
        ...
        await m.reply_document(out_path, caption="🔥 Titles Fixed & Dashboard Ready!")
    await msg.delete()
    os.remove(path)
